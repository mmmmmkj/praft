...
type Raft struct {
	...
	// groupLeaderAddr is the current cluster leader Address
	groupLeaderAddr ServerAddress
	// LeaderID is the current cluster leader ID
	groupLeaderID   ServerID
	groupLeaderLock sync.RWMutex

	// groupLeaderCh is used to notify of leadership changes
	groupLeaderCh chan bool

	// groupLeaderState used only while state is groupleader
	groupLeaderState groupLeaderState
	...
	// groupLeadershipTransferCh is used to start a leadership transfer from outside of
	// the main thread.
	groupLeadershipTransferCh chan *leadershipTransferFuture
	...
	// leaderNotifyCh is used to tell leader that config has changed
	groupLeaderNotifyCh chan struct{}

	// followerNotifyCh is used to tell followers that config has changed
	groupFollowerNotifyCh chan struct{}
...
func NewRaft(conf *Config, fsm FSM, logs LogStore, stable StableStore, snaps SnapshotStore, trans Transport) (*Raft, error) {
	...
	r := &Raft{
		...
		groupLeadershipTransferCh: make(chan *leadershipTransferFuture, 1),
		leaderNotifyCh:            make(chan struct{}, 1),
		followerNotifyCh:          make(chan struct{}, 1),
		groupLeaderNotifyCh:       make(chan struct{}, 1),
		groupFollowerNotifyCh:     make(chan struct{}, 1),
...
// or the leader is unknown.
func (r *Raft) GroupLeaderWithID() (ServerAddress, ServerID) {
	r.groupLeaderLock.RLock()
	groupLeaderAddr := r.groupLeaderAddr
	groupLeaderID := r.groupLeaderID
	r.groupLeaderLock.RUnlock()
	return groupLeaderAddr, groupLeaderID
}

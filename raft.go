...
// groupLeaderState is state that is used while we are a leader.
type groupLeaderState struct {
	groupLeadershipTransferInProgress int32 // indicates that a leadership transfer is in progress.
	commitCh                          chan struct{}
	commitment                        *commitment
	inflight                          *list.List // list of logFuture in log index order
	replState                         map[ServerID]*followerReplication
...
// setGroupLeader is used to modify the current leader Address and ID of the cluster
func (r *Raft) setGroupLeader(groupLeaderAddr ServerAddress, groupLeaderID ServerID) {
	r.groupLeaderLock.Lock()
	oldGroupLeaderAddr := r.groupLeaderAddr
	r.groupLeaderAddr = groupLeaderAddr
	oldgroupLeaderID := r.groupLeaderID
	r.groupLeaderID = groupLeaderID
...
func (r *Raft) runGroupFollower() {
	...
	for r.getState() == GroupFollower {
		...
		case l := <-r.groupLeadershipTransferCh:
			r.mainThreadSaturation.working()
			// Reject any operations since we are not the leader
		...
		case <-heartbeatTimer:
			...
			priorityThreshold := r.getPriorityThreshold()
			priority := r.getPriority()
			if priorityThreshold > priority {
				continue
			}
	...
	for r.getState() == GroupCandidate {
		...
		case vote := <-voteCh:
			...
			if grantedVotes >= votesNeeded {
				...
				r.setGroupLeader(r.localAddr, r.localID)
				return
			}
...
func (r *Raft) setGroupLeadershipTransferInProgress(v bool) {
	if v {
		atomic.StoreInt32(&r.groupLeaderState.groupLeadershipTransferInProgress, 1)
	} else {
		atomic.StoreInt32(&r.groupLeaderState.groupLeadershipTransferInProgress, 0)
	}
}

func (r *Raft) getGroupLeadershipTransferInProgress() bool {
	v := atomic.LoadInt32(&r.groupLeaderState.groupLeadershipTransferInProgress)
	return v == 1
}
...
func (r *Raft) setupGroupLeaderState() {
	...
	r.groupLeaderState.stepDown = make(chan struct{}, 1)
}

// runLeader runs the main loop while in leader state. Do the setup here and drop into
// the leaderLoop for the hot loop.
func (r *Raft) runGroupLeader() {

	r.logger.Info("entering leader state", "groupLeader", r)
	metrics.IncrCounter([]string{"raft", "state", "groupLeader"}, 1)
...
func (r *Raft) startStopReplicationForGroupLeader() {
	...
	for serverID, repl := range r.groupLeaderState.replState {
		...
		repl.stopCh <- lastIdx
		close(repl.stopCh)
		delete(r.groupLeaderState.replState, serverID)
		r.observe(PeerObservation{Peer: repl.peer, Removed: true})
	}
...
// index. This must only be called from the main thread.
// TODO
func (r *Raft) startStopReplicationForLeader() {
	inConfig := make(map[ServerID]bool, len(r.configurations.latest.ServersIsGroupLeader))
	lastIdx := r.getLastIndex()
...
func (r *Raft) groupLeaderLoop() {
	...
	for r.getState() == GroupLeader {
		...
		case <-r.groupLeaderState.stepDown:
			r.mainThreadSaturation.working()
			r.setState(GroupFollower)

		case future := <-r.groupLeadershipTransferCh:
			r.mainThreadSaturation.working()
			//TODO
			...
			r.logger.Debug("starting groupleadership transfer", "id", future.ID, "address", future.Address)

			// When we are leaving leaderLoop, we are no longer
			// leader, so we should stop transferring.
			leftGroupLeaderLoop := make(chan struct{})
			defer func() { close(leftGroupLeaderLoop) }()
			...
			go func() {
				defer r.setGroupLeadershipTransferInProgress(false)
				select {
			...
			state, ok := r.groupLeaderState.replState[*id]
			if !ok {
		...
		case <-r.groupLeaderState.commitCh:
			...
			if stepDown {
				if r.config().ShutdownOnRemove {
					...
					r.logger.Info("removed ourself, transitioning to groupFollower")
					r.setState(GroupFollower)
				}
			}
		//验证
		case v := <-r.verifyCh:
			...
			if v.quorumSize == 0 {
				...
				r.setState(GroupFollower)
				delete(r.groupLeaderState.notify, v)
				for _, repl := range r.groupLeaderState.replState {
					repl.cleanNotify(v)
				}
				v.respond(ErrNotLeader)
		...
		case <-lease:
			...
			maxDiff := r.checkGroupLeaderLease() //TODO
		...
		case <-r.groupLeaderNotifyCh:
			for _, repl := range r.groupLeaderState.replState {
				asyncNotifyCh(repl.notifyCh)
			}

		case <-r.groupFollowerNotifyCh:
...
func (r *Raft) verifyLeader(v *verifyFuture) {
	...
	v.quorumSize = r.quorumSizeForLeader()
	if v.quorumSize == 1 {
		v.respond(nil)
		return
	}
...
// verifyLeader must be called from the main thread for safety.
// Causes the followers to attempt an immediate heartbeat.
func (r *Raft) verifyGroupLeader(v *verifyFuture) {
	// Current leader always votes for self
	v.votes = 1

	// Set the quorum size, hot-path for single node
	v.quorumSize = r.groupQuorumSize()
	if v.quorumSize == 1 {
		v.respond(nil)
		return
	}
	...
	// Track this request
	v.notifyCh = r.verifyCh
	r.groupLeaderState.notify[v] = struct{}{}
...
func (r *Raft) checkGroupLeaderLease() time.Duration {
	// Track contacted nodes, we can always contact ourself
	contacted := 0
	...
	for _, server := range r.configurations.latest.ServersInGroup[r.groupId] {
		if server.Suffrage == Voter {
			...
			f := r.groupLeaderState.replState[server.ID]
			diff := now.Sub(f.LastContact())
			if diff <= leaseTimeout {
				contacted++
				if diff > maxDiff {
			...
			metrics.AddSample([]string{"raft", "groupleader", "lastContact"}, float32(diff/time.Millisecond))
		}
	}

	// Verify we can contact a quorum
	quorum := r.groupQuorumSize()
	if contacted < quorum {
		r.logger.Warn("failed to contact quorum of nodes, stepping down")
		r.setState(GroupFollower)
		metrics.IncrCounter([]string{"raft", "transition", "leader_lease_timeout"}, 1)
...
// the main thread.
// TODO: revisit usage
func (r *Raft) quorumSizeForLeader() int {
	voters := 0
	for _, server := range r.configurations.latest.ServersIsGroupLeader {
		if server.Suffrage == Voter {
			voters++
...
func (r *Raft) requestVote(rpc RPC, req *RequestVoteRequest) {
	...
	if r.getPriorityThreshold() > req.Priority {
		r.logger.Warn("rejecting vote request since our last index is greater",
			"candidate", candidate,
			"last-index", lastIdx,
			"last-candidate-index", req.LastLogIndex)
...
// electSelfInGroup is used to send a RequestVote RPC to all peers, and vote for
...
func (r *Raft) electSelfInGroup() <-chan *voteResult {
	...
	askPeer := func(peer Server) {
		r.goFunc(func() {
			defer metrics.MeasureSince([]string{"raft", "candidate", "electSelfInGroup"}, time.Now())
...
// response channel returned is used to wait for all the responses (including a
// vote for ourself). This must only be called from the main thread.
func (r *Raft) electSelfBetweenGroup() <-chan *voteResult {
	// Create a response channel
	respCh := make(chan *voteResult, len(r.configurations.latest.ServersIsGroupLeader))
...
// pickServer returns the follower that is most up to date and participating in quorum.
// Because it accesses leaderstate, it should only be called from the leaderloop.
func (r *Raft) pickGroupLeaderServer() *Server {
	var pick *Server
	var current uint64
	for _, server := range r.configurations.latest.ServersIsGroupLeader {
		if server.ID == r.localID || server.Suffrage != Voter {
...
func (r *Raft) pickGroupFollowerServer() *Server {
	...
	for _, server := range r.configurations.latest.ServersInGroup[r.groupId] {
		...
		state, ok := r.groupLeaderState.replState[server.ID]
		if !ok {
			continue
		}
		nextIdx := atomic.LoadUint64(&state.nextIndex)

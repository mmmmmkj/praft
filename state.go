...
const (
	...
	// GroupLeader is one of the valid states of a Raft node.
	GroupLeader
...
type raftState struct {
	...
	priorityThresholdLock sync.Mutex
...
func (r *raftState) getPriorityThreshold() (priorityThreshold float32) {
	r.priorityThresholdLock.Lock()
	priorityThreshold = r.priorityThreshold
	r.priorityThresholdLock.Unlock()
	return
}

func (r *raftState) setPriorityThreshold(priorityThreshold float32) {
	r.priorityThresholdLock.Lock()
	r.priorityThreshold = priorityThreshold
	r.priorityThresholdLock.Unlock()
}

func (r *raftState) getPriority() (priority float32) {
	r.priorityLock.Lock()
	priority = r.priority
	r.priorityLock.Unlock()
	return
}

func (r *raftState) setPriority(priority float32) {
	r.priorityLock.Lock()
	r.priority = priority
	r.priorityLock.Unlock()
}
